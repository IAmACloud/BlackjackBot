...
class VisionNode:
    def __init__(self):
        ...
        self.simulation_mode = bool(p.get('simulation_mode',False))  # 模拟模式默认开启

        rospy.loginfo(f"VisionNode init: simulation_mode={self.simulation_mode}, camera_topic={self.camera_topic}")
        ...
        if self.simulation_mode:
            ...
            self.collect_cards = True
            rospy.loginfo("Simulation mode: auto collect RPS and Cards")

    # ---------------- control callback ----------------
    def control_cb(self,msg:VisionControl):
        mode = msg.mode.strip().upper()
        rospy.loginfo(f"[control_cb] mode={mode}")
        if mode=="RPS_START":
            self.collect_rps = True
        elif mode=="RPS_STOP":
            self.collect_rps = False
    ...
    def image_cb(self,img_msg:Image):
        try:
            ...
            # 调试输出
            rospy.loginfo_once("Received first image from camera")
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            # ---- RPS ----
            if self.collect_rps:
                label = random.choice(['R','P','S']) if self.simulation_mode else 'R'  # 模拟模式随机
                conf = 0.9
            ...
            if self.collect_cards:
                card = random.choice(['AC','2C','KH']) if self.simulation_mode else 'AC'
                conf = 0.9
    ...
    def vote_rps(self):
        ...
        if ratio >= self.percent_required:
            ...
            rospy.loginfo(f"Published RPS: {top_label}, ratio={ratio:.2f}, conf={avg_conf:.2f}")
    ...
    def vote_cards(self):
        ...
        for card,cnt in counts.items():
            if cnt/len(buf) >= self.percent_required:
                ...
                rospy.loginfo(f"Published Card: {card}, ratio={cnt/len(buf):.2f}, conf={avg_conf:.2f}")
